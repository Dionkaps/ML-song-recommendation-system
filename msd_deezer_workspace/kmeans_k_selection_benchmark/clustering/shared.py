...
class PreparedDataset:
    ...
    reduction_mode: str = "pca_then_umap"


# Reduction modes available in the dim-reduction benchmark. Production code
# defaults to "pca_then_umap" for backwards compatibility with the original
# behaviour; the benchmark drivers iterate over all three.
REDUCTION_MODES = ("pca_only", "umap_only", "pca_then_umap")


def assert_inside_test_root(path: Path) -> None:
    """Defence-in-depth: refuse to write anywhere outside the test root.

    The benchmark workspace lives at .../msd_deezer_workspace/kmeans_k_selection_benchmark/
    and copies the production packages so their `WORKSPACE_DIR` resolves
    here. This guard double-checks: every output path passed through
    `ensure_output_dir` / `write_assignments` / `write_json` must be a
    descendant of WORKSPACE_DIR. If the copy ever drifts and starts
    pointing at the production tree, this trips before any I/O happens.
    """
    abs_path = Path(path).resolve()
    test_root = WORKSPACE_DIR.resolve()
    try:
        abs_path.relative_to(test_root)
    except ValueError as exc:
        raise RuntimeError(
            f"Refusing to write outside the K-selection benchmark root.\n"
            f"  attempted: {abs_path}\n"
            f"  test root: {test_root}\n"
            f"This guard exists to keep benchmark runs from polluting the "
            f"production pipeline directories."
        ) from exc
...
def build_summary_csv_from_raw(
    ...
    assert_inside_test_root(summary_csv_path)
...
def prepare_dataset(
    ...
    # UMAP defaults aligned to the official "for clustering" recommendations
    # (McInnes 2018; umap-learn docs/clustering.html): n_neighbors=30 weights
    # global structure, min_dist=0.0 packs cluster cores tightly. Production
    # code historically used (40, 0.01); both modes in the benchmark use the
    # documented values so the only thing differing across PCA/UMAP variants
    # is the presence or absence of the PCA pre-step.
    ...
    # `disable_umap` is the legacy switch that the production CLI scripts
    # already pass. `reduction_mode` is the new explicit selector; if a
    # caller passes both, the explicit one wins. If neither is set we fall
    # back to the production default of PCA -> UMAP.
    if reduction_mode is None:
        reduction_mode = "pca_only" if disable_umap else "pca_then_umap"
    if reduction_mode not in REDUCTION_MODES:
        raise ValueError(
            f"reduction_mode must be one of {REDUCTION_MODES}, got {reduction_mode!r}"
        )
    ...
    pca_reduced: np.ndarray | None = None

    needs_pca = reduction_mode in ("pca_only", "pca_then_umap")
    if needs_pca:
        max_components = min(
            max_pca_components, scaled_matrix.shape[0] - 1, scaled_matrix.shape[1],
        )
        if max_components < 2:
    ...
    if reduction_mode == "pca_only":
        reduced_matrix = pca_reduced
    elif reduction_mode == "umap_only":
        # Run UMAP directly on the StandardScaler output -- skip the PCA
        # pre-step entirely. Per the empirical counterpoint cited in the
        # 2026-04-25 calendar entry, PCA can strip nonlinear structure
        # before UMAP gets to model it; this mode is the test of whether
        # that matters on our features.
        umap_model = UMAP(
        ...
        reduced_matrix = umap_model.fit_transform(scaled_matrix)
        umap_dims = umap_n_components
        umap_neighbors = umap_n_neighbors
        umap_dist = umap_min_dist
    else:  # pca_then_umap
        if pca_reduced.shape[1] > umap_n_components:
            umap_model = UMAP(
                n_components=umap_n_components,
                n_neighbors=umap_n_neighbors,
                min_dist=umap_min_dist,
            ...
            # PCA already reduced to or below the UMAP target dim. Skipping
            # UMAP here matches production behaviour and avoids a degenerate
            # UMAP fit on tiny inputs.
            reduced_matrix = pca_reduced
...
def default_algorithm_output_dir(
    features_path: str | Path, algorithm: str, reduction_mode: str | None = None,
) -> Path:
    # Routing layout for the benchmark:
    #   cluster_results/<feature_source>/<reduction_mode>/<algorithm>/
    # When `reduction_mode` is None we fall back to the production layout
    # (cluster_results/<feature_source>/<algorithm>/) so the unmodified
    # production scripts in the parent workspace continue to work if they
    # ever import from this copy by accident.
    base = DEFAULT_CLUSTER_OUTPUT_DIR / feature_source_key(features_path)
    if reduction_mode is not None:
        base = base / reduction_mode
...
def write_assignments(assignments: pd.DataFrame, output_path: Path) -> None:
    ...
    assert_inside_test_root(output_path)

...
and materialise it inside `kmeans_k_selection_benchmark/audio/` as hard copies
(default) or symlinks (`--mode symlink`).

This is the only step in the benchmark that touches anything outside the
benchmark root, and it is strictly read-only with respect to the production
...
Safety:
  * Refuses to write to any path outside `kmeans_k_selection_benchmark/`.
  * Refuses to run if `kmeans_k_selection_benchmark/audio/` already exists and is
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sample N random songs from ../audio/ into kmeans_k_selection_benchmark/audio/. "
    ...
    parser.add_argument(
        ...
        help="Overwrite an existing non-empty benchmark audio/ directory.",
    )
    parser.add_argument(
...
def reset_dest(dest: Path, force: bool) -> None:
    dest = assert_inside_workspace(dest, "sample_audio_dir")
    if dest.exists():
...
def materialise(src: Path, dest: Path, mode: str) -> None:
    ...
    resolved_dest = assert_inside_workspace(dest, "sample_audio_file")
    try:
...
def main() -> None:
    ...
    manifest_path = assert_inside_workspace(args.manifest, "manifest")

...
from clustering.benchmark_metrics import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default=None, help="Directory for KMeans outputs. Defaults to cluster_results/<feature_source>/<reduction_mode>/kmeans.")
    ...
    # UMAP defaults aligned to McInnes' clustering recommendations: n_neighbors=30, min_dist=0.0.
    parser.add_argument("--umap-n-components", type=int, default=15, help="UMAP target dimensions for clustering.")
    parser.add_argument("--umap-n-neighbors", type=int, default=30, help="UMAP n_neighbors parameter (clustering default: 30).")
    parser.add_argument("--umap-min-dist", type=float, default=0.0, help="UMAP min_dist parameter (clustering default: 0.0).")
    parser.add_argument(
        "--reduction-mode", choices=list(REDUCTION_MODES), default="pca_then_umap",
        help="Dimensionality-reduction recipe: pca_only | umap_only | pca_then_umap.",
    )
    parser.add_argument("--disable-umap", action="store_true", help="Legacy alias for --reduction-mode pca_only.")
    parser.add_argument("--workers", type=int, default=16, help="Number of parallel workers for the KMeans grid search (default: 16).")
...
def _compute_benchmark_metrics(
    ...
    """Dunn + trustworthiness + bootstrap-ARI stability for the winning model."""
    ...
    # Trustworthiness: scaled (post-StandardScaler) -> reduced. n_neighbors=10
    # is the most common reporting choice; scaled vs reduced is the right
    # comparison because that's the actual reduction the algorithm saw.
    trust = trustworthiness_score(
        dataset.scaled_matrix, x_red,
        n_neighbors=10, sample_size=5000, random_state=random_state,
    )

    # Stability: refit at the SELECTED k on 5 bootstraps. Use the same
    # spherical (l2-normalised) input the production fit used.
    selected_k = int(model.n_clusters)

    def cluster_fn(x_sub: np.ndarray) -> np.ndarray:
        sub_norm = normalize(x_sub, norm="l2")
        m = KMeans(n_clusters=selected_k, random_state=random_state, n_init=10, max_iter=300)
        return m.fit_predict(sub_norm)

    stability = bootstrap_stability_ari(
        x_red, cluster_fn,
        n_bootstraps=5, sample_fraction=0.8, random_state=random_state,
    )
...
def main() -> None:
    args = parse_args()
    # Legacy --disable-umap alias takes precedence only if the explicit
    # --reduction-mode is left at its default. Surfaces a clear error if the
    # user passes contradictory flags.
    ...
    if args.disable_umap:
        if reduction_mode not in ("pca_then_umap", "pca_only"):
            raise SystemExit(
                ...
                f"Use --reduction-mode pca_only instead."
            )
        reduction_mode = "pca_only"

    resolved_output_dir = args.output_dir or default_algorithm_output_dir(
        args.features_path, "kmeans", reduction_mode=reduction_mode,
    )
    ...
    bench = _compute_benchmark_metrics(dataset, model, inlier_mask, args.random_state)
    payload.update(bench)
    payload["reduction_mode"] = reduction_mode
    write_json(payload, output_dir / "run_metadata.json")
    print(payload)

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default=None, help="Directory for GMM outputs. Defaults to cluster_results/<feature_source>/<reduction_mode>/gmm.")
...
def _compute_benchmark_metrics(
    ...
    # Stability: refit BGMM with the SAME hyperparameters on each bootstrap.
    cov_type = model.covariance_type
    n_comp = int(model.n_components)
    conc = float(model.weight_concentration_prior_)
    reg = float(model.reg_covar)
    ...
    def cluster_fn(x_sub: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            return np.zeros(len(x_sub), dtype=int)
...
def main() -> None:
    ...
    resolved_output_dir = args.output_dir or default_algorithm_output_dir(
        args.features_path, "gmm", reduction_mode=reduction_mode,
    )
    ...
    bench = _compute_benchmark_metrics(dataset, model, labels, args.random_state)
